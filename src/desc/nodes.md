...
- Массив пакетов
  ...
  Массив пакетов может быть сформирован только в [SQL yзле](#sql-узел) с помощью агрегатной функции `json_agg(json_build_object())`. Вложенные пакеты должны иметь одинаковую структуру.
